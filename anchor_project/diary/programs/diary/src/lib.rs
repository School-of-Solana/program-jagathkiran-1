...
declare_id!("BAzovQQqx3gzPXQmjZqbWMkeaAmncfQv4q39xu1vbznt");
...
pub mod diary {
    ...
    pub fn update_diary_entry(ctx: Context<UpdateEntry>, message: String) -> Result<()> {
        if message.as_bytes().len() > 840 {
            return Err(anchor_lang::error::ErrorCode::ConstraintRaw.into());
        }
    ...
    pub fn delete_diary_entry(ctx: Context<DeleteEntry>, title: String) -> Result<()> {
        if title.as_bytes().len() > 30 {
            return Err(anchor_lang::error::ErrorCode::ConstraintRaw.into());
        }
